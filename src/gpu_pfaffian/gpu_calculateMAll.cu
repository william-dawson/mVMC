...
extern "C" int CalculateMAll_real_gpu(const int *eleIdx, int qpStart, int qpEnd) {
  ...
  int *ipiv, *info;
  double *w, *scratch, *m, *uinv, *vt, *out;
  check_cuda(cudaMalloc(&ipiv, sizeof(int) * (size_t)n * batch), "cudaMalloc ipiv");
  check_cuda(cudaMalloc(&info, sizeof(int) * (size_t)batch), "cudaMalloc info");
  check_cuda(cudaMalloc(&w, sizeof(double) * (size_t)n * (2 * nb) * batch), "cudaMalloc w");
  check_cuda(cudaMalloc(&scratch, sizeof(double) * (size_t)(n - nb) * (n - nb) * batch), "cudaMalloc scratch");
  check_cuda(cudaMalloc(&m, sizeof(double) * (size_t)n * n * batch), "cudaMalloc m");
  check_cuda(cudaMalloc(&uinv, sizeof(double) * (size_t)(n - 1) * (n - 1) * batch), "cudaMalloc uinv");
  check_cuda(cudaMalloc(&vt, sizeof(double) * (size_t)n * batch), "cudaMalloc vt");
  check_cuda(cudaMalloc(&out, sizeof(double) * (size_t)n * n * batch), "cudaMalloc out");
